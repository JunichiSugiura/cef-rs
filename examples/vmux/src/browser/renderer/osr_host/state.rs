...
//! Winit / pump integration lives in [`super::lifecycle`], [`crate::window::dispatch`],
//! [`crate::window::pending_window_events`], and Bevy systems.
...
use crate::browser::event_loop::EditableFocusQueues;
...
pub struct OsrHostState {
    ...
    /// Last wins; applied in [`crate::browser::event_loop::apply_pending_set_active_browser_system`].
    /// `Cell` so pending set can run under `windows_store` guards.
    pub(crate) pending_set_active_browser: Cell<Option<i32>>,
    /// Drained by Bevy `Update` systems — see [`crate::browser::event_loop::EditableFocusQueues`].
    pub(crate) editable_focus_queues: EditableFocusQueues,
    /// Drained by [`crate::browser::shell_ops::apply_browser_ui_ops_system`].
...
impl OsrHostState {
    ...
    /// Resolves CEF `browser_id` for a [`WindowId`], preferring ECS [`BrowserId`](crate::browser::browser_entity::BrowserId) when it matches the CEF `windows_store`.
    pub(crate) fn browser_id_for_window(
        &self,
        window_id: WindowId,
        ecs: Option<(Entity, i32)>,
    ) -> Option<i32> {
        let store_bid = self.cef_attach.windows_store.lock().ok().and_then(|w| {
            w.get(&window_id).map(|e| e.browser.identifier())
        });
        match (ecs, store_bid) {
