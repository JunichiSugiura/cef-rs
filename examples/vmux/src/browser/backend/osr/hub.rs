...
pub struct PendingCefWindow {
    ...
    /// Initial navigation URL passed to `browser_host_create_browser` (often `about:blank`).
    pub url: String,
    /// When set, after the first load completes we navigate to this URL (staged startup for http(s)).
...
pub struct CefAttach {
    ...
    /// `browser_id` → URL to load after `about:blank` finishes (`apply_loading_state_changed_browser_events_system`).
