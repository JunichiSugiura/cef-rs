//! Editable-focus DOM probing for Vimium-style key routing in the embedded CEF view.
//!
//! - **CEF callbacks** (e.g. load finished) call [`schedule_editable_focus_probe`] directly so work is
//!   posted to the CEF UI thread without waiting for the next Bevy tick.
//! - **Winit shell** pushes into [`crate::browser::event_loop::EditableFocusQueues`]; Bevy
//!   `Update` drains those queues into [`EditableFocusProbeRequest`] /
//!   [`EditableVimiumReplayRequest`] events; [`dispatch_editable_focus_probe_requests_system`] and
//!   [`dispatch_editable_vimium_replay_requests_system`] run the CEF work.
...
#[derive(Event, Clone, Debug)]
pub struct EditableVimiumReplayRequest {
    pub browser_id: i32,
...
pub fn enqueue_vimium_replay_request(
    ...
    match tag.as_str() {
        ...
        _ => {
            ...
            let r = CefStringUtf8::from(&CefStringUtf16::from(&raw))
                .to_string()
                .to_lowercase();
            matches!(r.trim(), "textbox" | "searchbox" | "combobox" | "spinbutton")
        }
    }
}
...
pub fn run_editable_focus_probe_on_ui(browser_id: i32) {
    ...
    let browser = crate::browser::resolve_browser_for_editable_probe(browser_id);
...
pub(crate) fn dispatch_editable_vimium_replay_requests_system(
    mut requests: EventReader<EditableVimiumReplayRequest>,
) {
    for ev in requests.read() {
        post_editable_probe_for_vimium_replay(ev.browser_id, ev.window_id, ev.event.clone());
    }
}
...
pub fn refresh_editable_focus_hint_for_history(browser_id: i32) {
    schedule_editable_focus_probe(browser_id);
}

pub fn post_editable_probe_for_vimium_replay(
    browser_id: i32,
    window_id: WindowId,
    event: KeyEvent,
) {
    if currently_on(ThreadId::UI) != 0 {
        ...
        crate::browser::event_loop::send_user_event(
            crate::browser::event_loop::UserEvent::Vimium(
                crate::browser::event_loop::VimiumEvent::VimiumKeyReplay(
                    crate::browser::event_loop::VimiumKeyReplayEvent { window_id, event },
                ),
            ),
        );
        return;
    ...
    if post_task(ThreadId::UI, Some(&mut task)) == 0 {
        bevy_log::warn!(
            target: "vmux",
            pid = std::process::id(),
            "post_editable_probe_for_vimium_replay: post_task failed"
        );
    }
}
    ...
    impl Task {
        fn execute(&self) {
            ...
            crate::browser::event_loop::send_user_event(
                crate::browser::event_loop::UserEvent::Vimium(
                    crate::browser::event_loop::VimiumEvent::VimiumKeyReplay(
                        crate::browser::event_loop::VimiumKeyReplayEvent {
