...
define_class! {
    ...
    impl VmuxAppDelegate {
        ...
        unsafe fn create_application(&self, _object: Option<&AnyObject>) {
            ...
            assert!(app.isKindOfClass(VmuxApplication::class()));
            assert!(
                app.delegate()
                    .unwrap()
                    .isKindOfClass(VmuxAppDelegate::class())
            );
    ...
    unsafe impl NSUserInterfaceValidations for VmuxAppDelegate {
...
/// Instance variables of `VmuxApplication`.
#[derive(Default)]
...
pub fn setup_vmux_application() {
    // Initialize the VmuxApplication instance.
...
pub fn setup_vmux_app_delegate() -> Retained<VmuxAppDelegate> {
    ...
    let vmux_delegate = VmuxAppDelegate::new(mtm);
    let delegate_proto =
        ProtocolObject::<dyn NSApplicationDelegate>::from_retained(vmux_delegate.clone());
    let app = NSApp(MainThreadMarker::new().expect("Not running on the main thread"));
    assert!(app.isKindOfClass(VmuxApplication::class()));
    ...
    unsafe {
        vmux_delegate.performSelectorOnMainThread_withObject_waitUntilDone(
