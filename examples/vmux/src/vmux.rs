//! **vmux** uses **off-screen rendering (OSR)** + **winit** + **wgpu** (see `runtime`).
//!
...
//!    **`macos_lifecycle.rs`** uses **`#[name = "VmuxApplication"]`** so any tooling that resolves
...
//! [`cef::Settings::root_cache_path`] in bootstrap).
//!
...
fn append_vmux_core_chromium_switches(command_line: &mut CommandLine) {
    ...
    #[cfg(target_os = "macos")]
    {
        // Match `examples/osr` (no extra font/Skia switches here). Font-related flags caused
        // `EXC_BREAKPOINT` in fontations on macOS 26 + CEF 146 when combined with vmux's stack.
...
wrap_browser_process_handler! {
    ...
    impl BrowserProcessHandler {
        fn on_schedule_message_pump_work(&self, delay_ms: i64) {
            crate::browser::backend::cef::schedule_cef_work(delay_ms);
        }

        fn on_context_initialized(&self) {
            debug_assert_ne!(currently_on(ThreadId::UI), 0);
            // `cef::initialize` runs in `main` before the winit event loop (see `bootstrap::initialize_cef_after_execute`).
            // Client + headless wgpu are filled in Bevy `Startup` after that, not here.
        }

        /// Match [`examples/osr`]: propagate dev-oriented switches to GPU/renderer/utility children.
