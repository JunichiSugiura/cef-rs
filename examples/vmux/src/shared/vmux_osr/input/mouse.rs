...
pub fn wheel_to_cef(delta: MouseScrollDelta, mods: cef_event_flags_t) -> (f64, f64, cef_event_flags_t) {
    match delta {
        MouseScrollDelta::LineDelta(x, y) => (x as f64 * 120.0, y as f64 * 120.0, mods),
        MouseScrollDelta::PixelDelta(p) => {
            // Physical pixels; mark as precision scrolling.
            (p.x, p.y, mods | cef_event_flags_t::EVENTFLAG_PRECISION_SCROLLING_DELTA)
        }
    }
}
