...
wrap_render_handler! {
    pub struct VmuxOsrRenderHandler {
        inner: VmuxOsrRenderInner,
    }
    ...
    impl RenderHandler {
        fn view_rect(&self, browser: Option<&mut Browser>, rect: Option<&mut Rect>) {
            ...
            self.inner.hub.with_tab(id, |slot| {
                if let Ok(size) = slot.size.lock() {
                    // CEF expects view coordinates (DIP for OSR). Always set x/y and clamp.
                    rect.x = 0;
                    rect.y = 0;
        ...
        fn screen_info(
            ...
            if let Some(screen_info) = screen_info {
                let dsf = self
                    ...
                    .map(|v| *v)
                    .unwrap_or(1.0);
                screen_info.device_scale_factor = dsf;
                if let Some(browser) = browser {
                    let id = browser.identifier();
                    self.inner.hub.with_tab(id, |slot| {
                        if let Ok(size) = slot.size.lock() {
                            // ScreenInfo rects are in pixels. Use current view size * scale.
                            let w_px = (size.width.max(1.0) * dsf).ceil() as i32;
                            let h_px = (size.height.max(1.0) * dsf).ceil() as i32;
        ...
        fn on_accelerated_paint(
            ...
            let staging = self.inner.device.create_texture(&wgpu::TextureDescriptor {
                ...
                format: color::OSR_TEXTURE_BASE_FORMAT_BGRA,
                usage: wgpu::TextureUsages::TEXTURE_BINDING | wgpu::TextureUsages::COPY_DST,
                view_formats: &color::OSR_TEXTURE_VIEW_FORMATS_BGRA,
            });
                    ...
                    wgpu::BindGroupEntry {
                        ...
                        resource: wgpu::BindingResource::TextureView(&staging.create_view(
                            &wgpu::TextureViewDescriptor {
                                format: Some(color::OSR_TEXTURE_VIEW_FORMAT_BGRA),
                                ..Default::default()
                            },
                        )),
                    },
        ...
        fn on_paint(
            ...
            let (debug_paint, force_opaque) = if cfg!(debug_assertions) {
                (
                    std::env::var_os("VMUX_OSR_DEBUG_PAINT").is_some(),
                    std::env::var_os("VMUX_OSR_FORCE_OPAQUE").is_some(),
                )
            } else {
                (false, false)
            };
...
impl VmuxOsrRenderHandler {
    pub fn build(inner: VmuxOsrRenderInner) -> RenderHandler {
        Self::new(inner)
    }
}
